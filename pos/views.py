...
def cashier(request):
    """หน้า POS Cashier"""
    products = Product.objects.filter(quantity__gt=0, is_deleted=False)
...
def sales_history(request):
    qs = POSSale.objects.all().order_by('-created_at')  # หรือ filter(is_cancelled=False) ถ้าต้องการ
    ...
    if d_to:
        # end of day (inclusive): < next day 00:00
        end_next = timezone.make_aware(datetime.combine(d_to + timedelta(days=1), time.min), tz)
        qs = qs.filter(created_at__lt=end_next)

    return render(request, 'dashboard/order.html', {'sales': qs, **_topnav_stats()})
